...
public sealed class EventCatalogTests
{
    ...
    public void EventCatalog_ShouldDocumentEveryOwsEventTypeWithCorrectStatus()
    {
        ...
        foreach (OwsEventType type in Enum.GetValues<OwsEventType>())
        {
            ...
            var isActive = type is OwsEventType.FileCreated or OwsEventType.FileModified or OwsEventType.FileDeleted;
            if (isActive)
            {
                // Should be documented as Active
                catalogText.Should().Contain($"| `{typeName}` | **Active**", $"'{typeName}' is an active event type and must be marked Active in EVENT_CATALOG.md");
            }
            else
            {
                // Should be documented as Reserved
                catalogText.Should().Contain($"| `{typeName}` | **Reserved**", $"'{typeName}' is a reserved event type and must be marked Reserved in EVENT_CATALOG.md");
            }
        }
    }
