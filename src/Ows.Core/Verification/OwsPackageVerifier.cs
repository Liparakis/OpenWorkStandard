...
public sealed class OwsPackageVerifier : IPackageVerifier
{
    ...
    private static readonly VerificationFinding LargeUnobservedChangeFinding = new()
    {
        ...
        ReviewerAction = "OWS was not observing this project during the interval below. During that interval, a large file change appeared. OWS can verify the current package hashes, but cannot verify the unobserved edit process. OWS cannot determine the cause. This is not proof of misconduct. Reviewers should ask the student to explain this interval."
    };

    private static readonly VerificationFinding UnobservedChangeFinding = new()
    {
        ...
        ReviewerAction = "OWS was not observing this project during the interval below. During that interval, file changes appeared. OWS can verify the current package hashes, but cannot verify the unobserved edit process. This is not proof of misconduct. Reviewers should ask the student to explain this interval."
    };

    private static readonly VerificationFinding SnapshotMismatchFinding = new()
    {
        ...
        TechnicalDetail = "The local observed_snapshot.json state did not match the last SnapshotUpdated commitment, or the snapshot baseline could not be trusted during recovery.",
        ReviewerAction = "OWS could not verify that the local recovery snapshot matched the last committed snapshot state. OWS cannot safely reconstruct the exact unobserved file delta for this interval. This is not proof of misconduct. Reviewers should ask the student to explain this interval if relevant."
    };

    private static readonly VerificationFinding SnapshotUnboundFinding = new()
    {
        ...
        TechnicalDetail = "The local observed_snapshot.json existed without a matching SnapshotUpdated commitment, so OWS treated the baseline as legacy or untrusted for continuity reconstruction.",
        ReviewerAction = "OWS could not verify that the local recovery snapshot matched the last committed snapshot state. OWS cannot safely reconstruct the exact unobserved file delta for this interval. This is not proof of misconduct. Reviewers should ask the student to explain this interval if relevant."
    };
                ...
                while (reader.ReadLine() is { } line)
                {
                    ...
                    if (owsEvent != null)
                    {
                        if (owsEvent.EventType == OwsEventType.ObservationGapDetected)
                        {
                            ...
                            {
                                findings.Add(SnapshotUnboundFinding);
                            }
                            ...
                            {
                                findings.Add(SnapshotMismatchFinding);
                            }
                        }
                        else if (owsEvent.EventType == OwsEventType.UnobservedChangeDetected)
                        {
                            ...
                            findings.Add(new VerificationFinding
                            {
                                ...
                                Detail = $"A file change ({kind ?? "Modified"}: {bytesDeltaStr ?? "0"} bytes, {linesStr ?? "0"} lines) appeared while OWS was not observing this project.",
