...
classDiagram
    ...
    User "1" --> "*" Enrollment : enrolled via
    CourseOffering "1" --> "*" Enrollment : students/instructors
...
### 4. User and Enrollment
- **User**: Represents any participant (Student, Instructor, Admin) inside an institution.
- **Enrollment**: Connects a User to a specific CourseOffering under a designated role (`Student`, `Instructor`, `CourseAdmin`, `InstitutionAdmin`).
