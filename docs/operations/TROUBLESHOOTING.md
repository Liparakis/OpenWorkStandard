...
| **Request returns `429 rate_limit_exceeded`** | The verifier's built-in endpoint rate limiter rejected the request burst. | Check whether the failing path is auth-heavy, upload-heavy, or a public probe under load. Review `VerifierRateLimiting__*` settings and upstream proxy logs. | Slow the client retry loop or raise the specific limit if the deployment genuinely needs it. Do not disable rate limiting first. |
...
| **Package upload returns "entry path is not allowed", "duplicate archive entry", or "expands beyond the maximum safe limit"** | The package archive tripped the verifier's archive hardening checks. | Inspect the zip entry names, total expanded size, and compression ratios. Check `VerifierStorage__MaxPackageExpandedBytes` and `VerifierStorage__MaxPackageEntryCount`. | Rebuild the package from normal OWS output. If the package is legitimate but too large, raise the specific storage limits intentionally. |
