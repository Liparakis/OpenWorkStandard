...
| **Docker access-denied warnings appear, but verifier still starts** | The shell cannot talk to the Docker daemon, but PostgreSQL is already reachable at `localhost:5432`. | Check `.\scripts\status-local-verifier.ps1` and query `http://127.0.0.1:5078/ready`. | If `/ready` is `Ready`, you can continue. Otherwise start Docker Desktop or point `OWS_VERIFIER_CONNECTION_STRING` at a reachable PostgreSQL instance. |
