...
receipt_count="$(curl -fsS "$base_url/sessions/$session_id/receipts" | python -c "import json,sys; print(len(json.load(sys.stdin)['receipts']))")"
head_sequence="$(curl -fsS "$base_url/sessions/$session_id/head" | python -c "import json,sys; print(json.load(sys.stdin)['lastSequenceNumber'])")"
head_hash="$(curl -fsS "$base_url/sessions/$session_id/head" | python -c "import json,sys; print(json.load(sys.stdin)['lastTimelineHeadHash'])")"
